...
import com.udacity.catpoint.security.services.StyleService;
...
public class SensorPanel extends JPanel {
    ...
    private void updateSensorList(JPanel p) {
        p.removeAll();
        securityService.getSensors().stream().sorted(Comparator.comparing(s -> s.getSensorType() != null ? s.getSensorType().toString() : "")).forEach(s -> {
            String sensorType = (s.getSensorType() != null) ? s.getSensorType().toString() : "UNKOWN";
            JLabel sensorLabel = new JLabel(String.format("%s(%s): %s", s.getName(),sensorType  ,(s.getActive() ? "Active" : "Inactive")));
